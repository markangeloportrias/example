...
(function instructorProgressOverview() {
  ...
  async function openStudent(student, count, options = {}) {
    ...
    if (options.mount) {
      const parentContactDetail = byId('ipStudentInfo').querySelector('.ip-profile-details > div:last-child');
      parentContactDetail.querySelector('span').textContent = 'Parents Contact';
      parentContactDetail.querySelector('strong').textContent = student.parent_contact || 'Not provided';
    }
